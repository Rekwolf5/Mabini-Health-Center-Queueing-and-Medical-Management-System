...
<div class="queue-container">
    ...
    <div class="queue-item priority-{{ strtolower($item->priority) }} status-{{ strtolower($item->status) }}">
        <div class="queue-number">
            {{ $item->queue_number }}
        </div>
        <div class="queue-info">
            <h4>{{ $item->patient->full_name }}</h4>
            <p>Arrived: {{ $item->arrived_at->format('h:i A') }}</p>
        ...
        <div class="queue-status">
            ...
            @if($item->status === 'Waiting')
                <p><small>Waiting: {{ $item->arrived_at->diffForHumans() }}</small></p>
            @elseif($item->status === 'Consulting')
                <p><small>Started: {{ $item->started_at->diffForHumans() }}</small></p>
            @elseif($item->status === 'Completed')
                <p><small>Completed: {{ $item->completed_at->diffForHumans() }}</small></p>
            @endif
        </div>
        <div class="queue-actions">
            @if($item->status === 'Waiting')
            <form method="POST" action="{{ route('queue.updateStatus', $item->id) }}" style="display: inline;">
                @csrf
                @method('PATCH')
